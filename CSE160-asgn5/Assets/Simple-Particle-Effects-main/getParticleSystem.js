...
function getParticleSystem(params) {
  ...
  alphaSpline.addPoint(0.15, 0.55);
  alphaSpline.addPoint(0.55, 0.95);
  ...
  colorSpline.addPoint(0.0, new THREE.Color(0x123dff));
  colorSpline.addPoint(0.35, new THREE.Color(0x39a7ff));
  colorSpline.addPoint(0.75, new THREE.Color(0x7fe7ff));
  colorSpline.addPoint(1.0, new THREE.Color(0xd8f7ff));

  const sizeSpline = getLinearSpline((t, a, b) => a + t * (b - a));
  sizeSpline.addPoint(0.0, 0.0);
  sizeSpline.addPoint(0.2, 0.75);
  sizeSpline.addPoint(0.6, 1.0);
  ...
  let particleAccumulator = 0.0;

  function getEmitterWorldPosition() {
    const pos = new THREE.Vector3();
    emitter.getWorldPosition(pos);
    return pos;
  }

  function addParticles(timeElapsed) {
    particleAccumulator += timeElapsed;
    const n = Math.floor(particleAccumulator * rate);
    particleAccumulator -= n / rate;

    for (let i = 0; i < n; i++) {
      const life = (Math.random() * 0.45 + 0.55) * maxLife;
      const base = getEmitterWorldPosition();
      ...
      particles.push({
        ...
        size: (Math.random() * 0.45 + 0.55) * maxSize,
        colour: new THREE.Color(),
        alpha: 1.0,
        life: life,
        maxLife: life,
        rotation: Math.random() * 2.0 * Math.PI,
        rotationRate: Math.random() * 0.03 - 0.015,
        velocity: swirl,
      });
    }
  }
  ...
  function updateParticles(timeElapsed) {
    ...
    for (const p of particles) {
      ...
      drag.multiplyScalar(timeElapsed * 0.8);
      ...
      p.velocity.x += Math.sin(t * Math.PI * 2.0) * 0.002;
      p.velocity.z += Math.cos(t * Math.PI * 2.0) * 0.002;
    }
